    ...
    <>
      ...
      >
        ...
        <td className="px-5 py-3.5">
          ...
          <p className="text-muted text-[13px]">{order.customer_email ?? ''}</p>
        </td>
        <td className="px-5 py-3.5 text-right">
          <p className="text-parchment text-sm">SAR {Number(order.subtotal).toFixed(2)}</p>
        </td>
        <td className="px-5 py-3.5">
          <span className={`inline-block text-[12px] tracking-[0.15em] uppercase px-2 py-0.5 border ${statusPill(status)}`}>
        ...
        <td className="px-5 py-3.5 text-right">
          <span className="text-muted/40 text-[13px]">{expanded ? '▲' : '▼'}</span>
        </td>
      </tr>
        ...
        <tr className="border-b border-border bg-surface/40">
          <td colSpan={6} className="px-6 py-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              {/* Customer */}
              <div>
                <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-3">{t.detail_customer}</p>
                <div className="space-y-1.5">
                  <p className="text-parchment text-sm">{order.customer_name ?? '—'}</p>
                  {order.customer_phone && <p className="text-muted text-sm">{order.customer_phone}</p>}
                  {order.customer_email && <p className="text-muted text-sm">{order.customer_email}</p>}
                  {addr && (
              ...
              {/* Items */}
              <div>
                <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-3">{t.detail_items}</p>
                <div className="space-y-1.5">
                  ...
                  <div className="border-t border-border pt-2 mt-1 flex justify-between">
                    <p className="text-[13px] text-muted">{t.detail_subtotal}</p>
                    <p className="text-sm text-parchment">SAR {Number(order.subtotal).toFixed(2)}</p>
                  </div>
                </div>
              </div>
              ...
              {order.creator_link_id && order.creator_links && (
                <div className="md:col-span-3 border-t border-border pt-4 mt-2">
                  <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-2">{t.detail_attribution}</p>
                  <div className="flex gap-6">
                    <div>
                      <p className="text-[12px] text-muted/50 uppercase tracking-[0.15em] mb-1">{t.detail_link_code}</p>
                      <p className="text-parchment text-sm font-mono">{order.creator_links.link_code}</p>
                    </div>
                    <div>
                      <p className="text-[12px] text-muted/50 uppercase tracking-[0.15em] mb-1">{t.detail_commission}</p>
                      <p className="text-parchment text-sm">{order.creator_links.commission_rate}%</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Actions */}
              <div>
                <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-3">{t.detail_actions}</p>
                <div className="space-y-2">
                  ...
                  {status === 'pending' && (
                    <>
                      <button
                        ...
                        className="w-full text-[13px] tracking-[0.2em] uppercase px-4 py-2.5 bg-gold text-ink font-medium hover:bg-gold-light transition-colors disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="w-full text-[13px] tracking-[0.2em] uppercase px-4 py-2.5 border border-red-500/30 text-red-400 hover:border-red-400 transition-colors disabled:opacity-50"
                      >
                        {t.btn_cancel}
                      </button>
                    </>
                    ...
                    <>
                      <div onClick={e => e.stopPropagation()}>
                        <input
                          ...
                          className="w-full bg-surface border border-border text-parchment text-sm px-3 py-2 outline-none focus:border-gold/50 mb-2 font-mono placeholder:text-muted/40"
                        />
                      </div>
                      <button
                        disabled={isPending}
                  ...
                  {status === 'shipped' && (
                    <button
                      ...
                      className="w-full text-[13px] tracking-[0.2em] uppercase px-4 py-2.5 bg-green-900/30 border border-green-500/30 text-green-400 hover:bg-green-900/50 transition-colors disabled:opacity-50"
                    >
                  ...
                  {!['pending', 'confirmed', 'shipped'].includes(status) && (
                    <p className="text-muted text-sm">{t.detail_no_actions}</p>
                  )}
                </div>
              </div>
            ...
            {order.brand_notes && (
              <div className="mt-5 pt-5 border-t border-border">
                <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-1.5">{t.detail_notes}</p>
                <p className="text-muted text-sm">{order.brand_notes}</p>
              </div>
            )}
          </td>
        </tr>
...
export function BrandOrdersClient({ orders, locale = 'en' }: { orders: OrderRow[]; locale?: DashLang }) {
  ...
  return (
    <div className="p-6 md:p-10">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="text-[13px] text-gold tracking-[0.3em] uppercase mb-1">{t.eyebrow}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="border border-border border-t-2 border-t-border/60 p-4">
          <p className="text-[12px] text-muted tracking-[0.2em] uppercase mb-2">{t.stat_total}</p>
        ...
        <div className="border border-border p-4" style={{ borderTop: '2px solid rgba(234,179,8,0.4)' }}>
          <p className="text-[12px] text-muted tracking-[0.2em] uppercase mb-2">{t.stat_pending}</p>
        ...
        <div className="border border-border p-4" style={{ borderTop: '2px solid rgba(168,85,247,0.4)' }}>
          <p className="text-[12px] text-muted tracking-[0.2em] uppercase mb-2">{t.stat_shipped}</p>
        ...
        <div className="border border-border p-4" style={{ borderTop: '2px solid rgba(74,222,128,0.4)' }}>
          <p className="text-[12px] text-muted tracking-[0.2em] uppercase mb-2">{t.stat_revenue}</p>
      ...
      <div className="flex gap-1.5 flex-wrap mb-6">
        {TABS.map(tabKey => (
          <button
            ...
            className={`text-[12px] tracking-[0.2em] uppercase px-3 py-1.5 border transition-colors ${
              tab === tabKey
      ...
      {filtered.length === 0 ? (
        <div className="border border-border p-10 text-center">
          <p className="text-muted text-base">{t.empty_tab(tab)}</p>
        </div>
      ) : (
            ...
            <thead>
              <tr className="border-b border-border bg-surface/50">
                {([t.col_order, t.col_customer, t.col_total, t.col_status, t.col_date, ''] as string[]).map((h, i) => (
                  <th key={i} className={`px-5 py-3 text-[12px] text-muted tracking-[0.2em] uppercase font-normal ${
