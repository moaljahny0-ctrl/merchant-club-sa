  ...
  return (
    <div className="mb-8 border border-border px-6 py-5">
      <div className="flex items-end justify-between mb-5">
        <div>
          <p className="text-[12px] text-gold tracking-[0.3em] uppercase mb-1">{t.design_heading}</p>
          <p className="text-muted text-sm">{t.design_body}</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isPending}
      ...
      {/* Template picker */}
      <p className="text-[12px] text-muted/60 tracking-[0.2em] uppercase mb-2">{t.template_label}</p>
      <div className="flex gap-2 mb-5">
        {TEMPLATES.map(tpl => (
          <button
            key={tpl.id}
            onClick={() => { setTemplateId(tpl.id); setSaved(false) }}
            className={`text-[13px] tracking-[0.15em] uppercase px-4 py-2 border transition-colors ${
      ...
      {/* Accent color picker */}
      <p className="text-[12px] text-muted/60 tracking-[0.2em] uppercase mb-2">{t.accent_label}</p>
      <div className="flex gap-2 mb-5">
        {palette.map(color => (
          <button
            key={color.id}
      ...
      {/* Social links */}
      <p className="text-[12px] text-muted/60 tracking-[0.2em] uppercase mb-2">{t.social_heading}</p>
